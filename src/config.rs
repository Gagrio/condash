...
pub fn setup_temp_dir(_container_name: &str) -> Result<PathBuf> {
    ...
    let datasource_content =
        include_str!("../configs/grafana/provisioning/datasources/prometheus.yaml");
    fs::write(
    ...
    let dashboard_config =
        include_str!("../configs/grafana/provisioning/dashboards/dashboards.yaml");
    fs::write(
